...
import { Mic2, Music, Palette, ArrowRight, ChevronRight } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-16">
      ...
      <div className="flex flex-col gap-4 md:hidden">
        {courses.map((course) => (
          <div 
            key={course.id} 
            className={`bg-white rounded-2xl p-2.5 shadow-lg border-2 ${course.border} flex gap-3 items-stretch animate-fade-in active:scale-[0.98] transition-transform`}
          >
            {/* Small Left Image */}
            <div 
              onClick={() => onViewDetails?.(course.id)}
              className="relative w-24 shrink-0 rounded-xl overflow-hidden cursor-pointer"
            >
            ...
            <div className="flex flex-col justify-between flex-grow py-1">
              <div onClick={() => onViewDetails?.(course.id)}>
                ...
                <p className="text-xs text-gray-500 font-bold leading-snug line-clamp-2">
                  {course.description}
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 mt-2">
                <button 
                  onClick={() => onViewDetails?.(course.id)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-wide border ${course.btnBorder} ${course.btnBg}`}
                >
                  Detalii
                </button>
                <button 
                  ...
                  className={`flex-1 py-1.5 rounded-lg text-xs font-black text-white bg-gradient-to-r ${course.color} shadow-sm flex items-center justify-center gap-1`}
                >
                  Înscriere <ChevronRight size={12} />
                </button>
              </div>
            </div>
          </div>
